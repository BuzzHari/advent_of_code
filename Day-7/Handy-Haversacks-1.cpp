...
//int dfs(int vertex, const vector<pair<int,int>>& edges,int visited[], int my_bag_id) {
int dfs(int vertex, const vector<pair<int,int>>& edges, int visited[], int my_bag_id);
...
int main(int argc, char **argv) {
    ...
    int bag_count = 0, ret;
    for (auto it = rules.begin(); it != rules.end(); ++it) {
        ret = dfs(it->first, it->second, visited, my_bag_id);
        if (ret)
            ++bag_count;
    }
...
int dfs(int vertex, const vector<pair<int,int>>& edges,int visited[], int my_bag_id) {
    ...
    for (int i = 0; i < edges.size(); ++i) {
        if (edges[i].first == my_bag_id)
            return 1;
        else {
            int ret = dfs(edges[i].first, rules[edges[i].first], visited, my_bag_id);
            if (ret)
                return 1;
        }
    }
